...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      const res = await deleteTrainSeat(row.id)
      if (res.code === 200) {
        ElMessage.success('删除成功')
        // 如果当前页只有一条数据且不是第一页，删除后自动跳转到上一页
        if (trainSeatList.value.length === 1 && pagination.page > 1) {
          pagination.page--
        }
        await getTrainSeatList()
      } else {
...
const getTrainSeatList = async () => {
  ...
  try {
    ...
    console.error('获取车厢座位列表失败:', error)
    ElMessage.error('获取车厢座位列表失败')
  } finally {
